...
void loop()
{
  ...
  // Verify that delay() doesn't interfere with timing:
  // delay(2000);
  // -> NO effect (as expected)

  //* Verify that counter contiues while asleep:
  // LowPower.sleep(2500);
  // delay(2500);
  //*  If counter doesn't continue while asleep, would expect a blink period of ~ 10 seconds
  //*  This is what is happening! So the counter does not count when asleep, while try using different timers to see if that makes a difference:
